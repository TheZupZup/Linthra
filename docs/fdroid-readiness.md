...
| **Native components** | SQLite, via `sqlite3_flutter_libs` — compiled **from source** (the SQLite amalgamation is public domain) using the NDK + CMake; and `libflacJNI.so`, Media3's FLAC decoder built from the vendored `third_party/libflac` (BSD-3-Clause) and `third_party/media3_decoder_flac` (Apache-2.0) sources with the same NDK + CMake (#674). No prebuilt closed blob. The playback engine is AndroidX Media3 (Apache-2.0 Maven AAR), not GMS. |
