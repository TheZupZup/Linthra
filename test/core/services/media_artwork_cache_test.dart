...
void main() {
  ...
  group('MediaArtworkCache cacheKeyReference (size-safe keys)', () {
    // A transform that folds a size marker into the cache identity — the same
    // shape the media-session wiring uses to size-tag Plex covers.
    Uri tagSize(Uri reference) {
      return reference.replace(
        queryParameters: <String, String>{'mscover': '512'},
      );
    }

    test('keys the cached file under the transformed identity', () async {
      final cache = MediaArtworkCache(
        resolveUrl: (Uri reference) => _authUrl,
        cacheKeyReference: tagSize,
        fetch: (Uri url) async => _imageBytes,
        directory: directory,
      );
      ...
      // The filename hashes the *size-tagged* identity, not the bare reference,
      // so a different size can never reuse these bytes (no wrong-size reuse).
      final String tagged =
          '${sha256.convert(utf8.encode(tagSize(_reference).toString()))}.img';
      final String bare =
          '${sha256.convert(utf8.encode(_reference.toString()))}.img';
      expect(result!.pathSegments.last, tagged);
      expect(result.pathSegments.last, isNot(bare));
    });

    test('cached() finds the entry via the original reference', () async {
      final cache = MediaArtworkCache(
        resolveUrl: (Uri reference) => _authUrl,
        cacheKeyReference: tagSize,
        fetch: (Uri url) async => _imageBytes,
      ...
      final Uri? resolved = await cache.resolve(_reference);
      // The handler reads with the original reference; the transform is applied
      // internally, so the warmed cover is still found under the same key.
      expect(cache.cached(_reference), resolved);
    });

    test('coverReady still emits the original (untransformed) reference',
        () async {
      final cache = MediaArtworkCache(
        resolveUrl: (Uri reference) => _authUrl,
        cacheKeyReference: tagSize,
      ...
      // The audio handler matches coverReady against the track's artworkUri (the
      // original reference), so the keying transform must not leak into it.
      expect(ready, <Uri>[_reference]);
    });
  });
}
